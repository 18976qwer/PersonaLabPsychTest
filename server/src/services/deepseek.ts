...
export const generateDeepSeekReport = async (
  ...
  const allModules = {
    ...
    growth: `
      "growth": { 
          "best": { "state": "最佳状态", "work": "严格输出3条工作表现，每条使用“标签：具体描述”格式，每条不超过20字，必须精炼（关键词风格），条目之间只能使用中文全角分号或换行分隔，禁止合并为一整段文字", "rel": "严格输出3条亲密关系表现，格式与长度要求与work完全一致（20字以内）", "feeling": "用1句话概括此状态下的内在感受，总字数不超过25字，一针见血", "how": "进入该状态的关键路径，给出2-3条建议，每条不超过20字，条目之间使用分号或换行分隔" },
          "daily": { "state": "日常状态", "work": "严格输出3条工作表现，格式与长度要求与best.work完全一致（20字以内）", "rel": "严格输出3条亲密关系表现，格式与长度要求与best.rel完全一致（20字以内）", "feeling": "用1句话概括此状态下的内在感受，总字数不超过25字，避免堆砌形容词", "how": "保持或优化该状态的建议，突出关键抓手，总字数不超过25字" },
          "stress": { "state": "压力状态", "work": "严格输出3条工作表现，格式与长度要求与best.work完全一致（20字以内），重点描述压力下的具体行为", "rel": "严格输出3条亲密关系表现，格式与长度要求与best.rel完全一致（20字以内），重点描述压力下的互动模式", "feeling": "用1句话描述此状态下的典型情绪体验，总字数不超过25字", "how": "识别并调节该状态的具体方法，给出2-3条练习，每条不超过20字" }
      }`,
    relationships: `
      "relationships": {
          "communication": [
  ...
  try {
    ...
    // You might want to throw a specific error or return null
    throw error;
  }
};
